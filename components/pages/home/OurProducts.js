...
export default function OurProducts(){
    ...
    return(
        <Flex w="100%" flexDir="column" alignItems="center" gridGap="2rem" py="3rem" bg="white">
            <Heading color="primary.black" borderBottom="5px solid #F18720">Our Products</Heading>
            <Text color="primary.black" fontWeight={400} fontSize="0.85em" textAlign="center" maxW="40%">We have a lineup of product that are proven and are used throughout the
nation in many capacity. </Text>
            <Grid       gridTemplateColumns="repeat(4, 1fr)" gridGap={10}>
                {datas.map((item) =>
                (
                    <GridItem>
                    <ProductCard img={item.img} text={item.text} hover={item.hover}/> 

                    </GridItem>
                )
                )}
            </Grid>
            <Box py="0.3rem" bg="primary.orange" borderRadius="20px" boxShadow="0px 22px 40px rgba(0, 0, 0, 0.15)" w="15%" textAlign="center" fontSize='0.85em' fontWeight={600} cursor="pointer" color="white" _hover={{bg:"#D26801"}}>See all our products</Box>
        </Flex>
    );
}